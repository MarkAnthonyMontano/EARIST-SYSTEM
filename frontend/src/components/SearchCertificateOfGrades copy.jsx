...
const SearchCertificateOfGrades = () => {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        console.error("Error fetching program table:", err);
      }
    };
  ...
  const handleSearchStudent = async () => {
    ...
    try {
      ...
      const programMatched = programs.find(
        (item) => item.program_description === course_desc
      );
      const derivedMajor = programMatched?.major || response.data.major || "";

      setMajor(derivedMajor);
                        ...
                        <td colSpan={16} style={{ fontSize: "62.5%" }}>
                          <input
                            ...
                            value={
                              (
                                programs.find(
                                  (item) => item.program_code === courseCode // or use curriculum_id if needed
                                )?.program_description || ""
                              )
                            }
